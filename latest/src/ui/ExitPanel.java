...
public class ExitPanel extends JPanel {
    ...
    private void calculateExit() {
        try {
            ...
            if (mins > 1440) {
                long overstayMins = mins - 1440;
                currentFine = fineStrategy.calculateFine(overstayMins);
                fineDescription = "Overstay fine (" + (overstayMins/60) + " hrs over 24hr limit)";
            }

            // Reserved spot misuse
            if (spot != null && spot.getType().equalsIgnoreCase("Reserved")) {
                boolean hasReservation = false; // Simplified - assume no
                if (!hasReservation) {
                    currentFine += 50.0;
                    fineDescription = fineDescription.isEmpty() ?
                            "Reserved spot without reservation" :
                            fineDescription + " + Reserved spot fine";
                }
            }
        ...
        try {
            ...
            if (amountPaid > 0 && currentFine > 0) {
                ...
                if (paymentForCurrentFine < currentFine) {
                    ...
                    paymentDAO.createFine(ticket.getTicketId(), plate, remainingFine, 0, "Unpaid Fine");
                }
